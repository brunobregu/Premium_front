...
export default function CreateEditShipment({
  ...
  const addOrderDetailsDtoSchema = yup.object().shape({
    ...
    paymentStatus: yup
      .string()
      .required(i18n.t('validation.paymentStatus-required')),
    partlyPaid: yup
      .number()
      .transform((value) => (Number.isNaN(value) ? null : value))
      .integer(),
    ...
    <div className="@container">
      <FormProvider {...methods}>
        <form onSubmit={methods.handleSubmit(onSubmit)}>
              ...
              render={({ field: { value, onChange } }) => (
                <Select
                  ...
                  displayValue={(selected) =>
                    auctionOptions.find(
                      (c: any) => c.label.toString() === selected
          ...
          <hr />
          <div className="mb-4 mt-4 flex flex-col items-start gap-4 md:flex-row md:items-center md:justify-start">
            <p className="text-xl">
              {' '}
              {i18n.t('car-status')}:{' '}
              <span className="font-bold ">
                {orderDetailsQuery.data?.data.carStatus
                  ? orderDetailsQuery.data?.data.carStatus
                  : 'Dispatch'}{' '}
              </span>{' '}
            </p>
          ...
          <Button
            type="submit"
            className="mx-auto flex  flex-1 bg-gray-900 !px-14 !py-[.6rem] text-white hover:bg-gray-800 sm:justify-center md:mx-0 md:justify-start"
            isLoading={isLoading}
            disabled={isLoading}
          >
